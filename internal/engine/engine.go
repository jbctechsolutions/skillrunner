...
func (sr *Skillrunner) ListSkills() ([]types.SkillConfig, error) {
	...
	// Add orchestrated skills from skills directory
...
// Precedence order:
// 1. Config override (cfg.Paths.SkillrunnerDir/skills)
// 2. Default location (~/.skillrunner/skills)
// 3. Workspace-relative (workspace/skills) - for project-local skills
func (sr *Skillrunner) getOrchestratedSkillsDir() string {
	// 1. Check if path is overridden in config
	cfgManager, err := config.NewManager("")
	if err == nil {
		cfg := cfgManager.Get()
		if cfg.Paths != nil && cfg.Paths.SkillrunnerDir != "" {
			configDir := filepath.Join(cfg.Paths.SkillrunnerDir, "skills")
	...
	// 2. Default to ~/.skillrunner/skills
	homeDir, err := os.UserHomeDir()
	if err == nil {
		defaultDir := filepath.Join(homeDir, ".skillrunner", "skills")
		if _, err := os.Stat(defaultDir); err == nil {
			return defaultDir
		}
	}

	// 3. Fall back to workspace/skills for project-local skills
	workspaceDir := filepath.Join(sr.workspacePath, "skills")
	if _, err := os.Stat(workspaceDir); err == nil {
		return workspaceDir
	}

	// Return default even if it doesn't exist (will fail gracefully when loading)
...
func (sr *Skillrunner) GetStatus() (*types.SystemStatus, error) {
	...
	if err != nil {
		// Model providers may be unavailable (e.g., Ollama not running)
		// This shouldn't fail the entire status command
		configuredModels = []string{fmt.Sprintf("(unable to list models: %v)", err)}
		ready = false
	} else {
...
func (sr *Skillrunner) loadSkill(skillName string) (*types.SkillConfig, error) {
	...
	// Try orchestrated skills from skills directory
	skillsDir := sr.getOrchestratedSkillsDir()
	loader := orchestration.NewSkillLoader(skillsDir)
...
func NewSkillrunnerForTesting(workspace string, policy models.ResolvePolicy) *Skillrunner {
	if workspace == "" {
		workspace = "."
	}
	if policy == "" {
