...
func (m *Manager) SetAPIKey(provider, key string) error {
	...
	case "openai":
		m.config.APIKeys.OpenAI = key
	case "google":
		m.config.APIKeys.Google = key
	default:
...
func (m *Manager) GetAPIKey(provider string) string {
	...
	case "openai":
		return m.config.APIKeys.OpenAI
	case "google":
		return m.config.APIKeys.Google
	default:
		return ""
	}
}
...
func (m *Manager) GetEnvVars() map[string]string {
	...
	} else if m.config.APIKeys.OpenAI != "" {
		env["OPENAI_API_KEY"] = m.config.APIKeys.OpenAI
	}

	if val := os.Getenv("GOOGLE_API_KEY"); val != "" {
		env["GOOGLE_API_KEY"] = val
	} else if m.config.APIKeys.Google != "" {
		env["GOOGLE_API_KEY"] = m.config.APIKeys.Google
	}
