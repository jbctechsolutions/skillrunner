...
func TestCreateOpenAIProvider(t *testing.T) {
	tests := []struct {
		...
		{
			name: "new config format enabled",
			config: &config.Config{
				Providers: &config.Providers{
		...
		{
			name: "new config format disabled",
			config: &config.Config{
				Providers: &config.Providers{
					OpenAI: &config.OpenAIConfig{
						APIKey:  "sk-test-key",
...
func TestCreateOpenRouterProvider(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "enabled with custom base URL",
			config: &config.Config{
				Providers: &config.Providers{
		...
		{
			name: "enabled with default base URL",
			config: &config.Config{
				Providers: &config.Providers{
					OpenRouter: &config.OpenRouterConfig{
						APIKey:  "${TEST_OR_KEY}",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			provider, err := createOpenRouterProvider(tt.config)
			...
			if provider != nil && !tt.expectNil {
				...
				if openaiProvider.baseURL != tt.expectedURL {
					t.Errorf("expected baseURL %q, got %q", tt.expectedURL, openaiProvider.baseURL)
				}
			}
		})
	}
...
func TestCreateGroqProvider(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			provider, err := createGroqProvider(tt.config)
			...
			// Verify Groq uses correct base URL
			if provider != nil && !tt.expectNil {
				openaiProvider, ok := provider.(*OpenAIProvider)
				if !ok {
					t.Fatal("provider is not an OpenAIProvider")
				...
				if openaiProvider.baseURL != expectedURL {
					t.Errorf("expected baseURL %q, got %q", expectedURL, openaiProvider.baseURL)
				}
			}
		})
	}
