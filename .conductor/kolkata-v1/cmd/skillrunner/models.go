...
func createOrchestrator() (*models.Orchestrator, error) {
	...
	openaiKey := cfgManager.GetAPIKey("openai")
	if openaiKey == "" {
		openaiKey = os.Getenv("OPENAI_API_KEY")
	}
	if openaiKey != "" {
		openaiProvider, err := models.NewOpenAIProvider(openaiKey, "", nil)
		if err == nil {
			orchestrator.RegisterProvider(openaiProvider)
		} else {
			fmt.Fprintf(os.Stderr, "Warning: Failed to initialize OpenAI provider: %v\n", err)
		}
	}
