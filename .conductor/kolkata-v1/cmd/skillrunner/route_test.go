...
// getTestSkillsDir returns the path to the test fixtures directory
func getTestSkillsDir() string {
	_, filename, _, _ := runtime.Caller(0)
	// Navigate from cmd/skillrunner to internal/engine/testdata/skills
	return filepath.Join(filepath.Dir(filename), "..", "..", "internal", "engine", "testdata", "skills")
}
...
func setupTestEngine(workspace string, policy models.ResolvePolicy) *engine.Skillrunner {
	// Create engine with test skills directory to load from test fixtures
	eng := engine.NewSkillrunnerForTesting(workspace, policy, getTestSkillsDir())
...
func TestCommandRouter_RouteList(t *testing.T) {
	...
	t.Run("table format", func(t *testing.T) {
		...
		// Note: Test skills registered via RegisterTestSkill are only in the cache
		// for loadSkill(), not in ListSkills(). The table format is validated by
